...
/// まちがいおに撃破リザルト — ヒカキン流
struct MistakeBossResultView: View {
    ...
    var body: some View {
        ZStack {
            ...
            if bgRevealed {
                LinearGradient(
                    colors: [Color(red: 0.8, green: 0.6, blue: 0), Color(red: 1.0, green: 0.8, blue: 0.2)],
            ...
            VStack(spacing: 24) {
                ...
                // Phase 0: タイトルドーン
                Text("mistake_boss_defeated_title")
                    .font(.zenMaru(30, weight: .black))
                    .foregroundStyle(.white)
                    .shadow(color: .black.opacity(0.3), radius: 8, y: 2)
                    .scaleEffect(titleScale).opacity(titleOpacity)

                // Phase 1: サブタイトル
                Text(MistakeBossConfig.defeatLines.randomElement() ?? "")
                ...
                // Phase 2: XP ドーン
                Text("+\(MistakeBossConfig.xpBonus) XP")
                ...
                // Phase 3: 褒め
                Text("mistake_boss_result_praise")
                ...
                Button {
                    HapticsManager.tap()
                    gameVM.isMistakeBossMode = false
                    gameVM.returnToTitle()
                } label: {
                    Text("mistake_boss_done")
                        .font(.zenMaru(22, weight: .bold))
                        .foregroundStyle(.white)
    ...
    private func runSequence() {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.15) {
            withAnimation(.easeOut(duration: 0.2)) { showFlash = false; bgRevealed = true }
        }
